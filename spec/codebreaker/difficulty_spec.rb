...
RSpec.describe Codebreaker::Difficulty do
  ...
  it 'receives the number of attempts on default difficulty' do
    expect(difficulty.attempts).to eq(Codebreaker::Difficulty::DIFFICULTIES[level][:attempts])
  end

  it 'receives the number of hints on default difficulty' do
    expect(difficulty.hints).to eq(Codebreaker::Difficulty::DIFFICULTIES[level][:hints])
  end
